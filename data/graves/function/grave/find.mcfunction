...
$execute if score owner graves.settings matches 1 as @n[tag=graves.grave.player.$(player_id),nbt={interaction:{}}] at @s run return run function graves:grave/found {"player_id":$(player_id)}